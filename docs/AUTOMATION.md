...
The project includes three main automation workflows:

1. **Package Manager Updates** - Automatically updates package configurations when releases are published
2. **Submit to Package Repositories** - Automates submissions to package repositories where APIs are available
3. **Test Package Installations** - Validates that packages install and work correctly

## Workflows

### 1. Package Manager Updates (`.github/workflows/package-managers.yml`)
...
### 2. Submit to Package Repositories (`.github/workflows/submit-packages.yml`)
...
### 3. Test Package Installations (`.github/workflows/test-packages.yml`)
...
2. **`HOMEBREW_TAP_TOKEN`** (Optional, for Homebrew tap updates)
...
1. **Create a release** using GitHub's release interface or git tags
2. **Package Manager Updates workflow** runs automatically
3. **Submit to Package Repositories workflow** runs automatically after updates complete
4. **Test Package Installations workflow** runs to validate everything works
...
gh workflow run package-managers.yml -f version=v0.1.0

# Submit to specific repositories
gh workflow run submit-packages.yml -f version=v0.1.0 -f repositories=crates,homebrew

# Test package installations
gh workflow run test-packages.yml -f version=v0.1.0
```
